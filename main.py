...
# ========== LANGUAGE SYSTEM ==========
current_language = 'kz'  # Default language: kz, uz, ru, en
lang_data = {}  # Language data storage
...
# ==================== LANGUAGE FUNCTIONS ====================
def load_language(lang_code='kz'):
    """Load language file from languages folder"""
    global lang_data, current_language
    try:
        lang_path = os.path.join(os.path.dirname(__file__), 'languages', f'{lang_code}.json')
        ...
        print(f"❌ Language file not found: {lang_code}.json")
        return False
...
def get_text(key_path, **kwargs):
    ...
    Example: get_text('commands.lang.current', lang='kz')
    """
    try:
        keys = key_path.split('.')
        value = lang_data
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        # ========= LANG =========
                        if inPuTMsG.startswith('/lang'):
                            parts = inPuTMsG.strip().split()

                            # Show current language if no argument
                            if len(parts) == 1:
                                current_msg = get_text('commands.lang.current', lang=current_language)
                                available_msg = get_text('commands.lang.available')
                                await safe_send_message(response.Data.chat_type, f"{current_msg}\n{available_msg}", uid, chat_id, key, iv)

                            # Change language
                            elif len(parts) == 2:
                                new_lang = parts[1].lower()
                                if new_lang in ['kz', 'uz', 'ru', 'en']:
                                    if load_language(new_lang):
                                        success_msg = get_text('commands.lang.changed', lang=new_lang)
                                        await safe_send_message(response.Data.chat_type, f"✅ {success_msg}", uid, chat_id, key, iv)
                                    else:
                                        await safe_send_message(response.Data.chat_type, "❌ Language file not found!", uid, chat_id, key, iv)
                                else:
                                    usage_msg = get_text('commands.lang.usage')
